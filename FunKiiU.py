...
def download_file(url, outfname, retry_count=3, ignore_404=False, expected_size=None, chunk_size=0x4096):
    for _ in retry(retry_count):
        try:
            infile = urlopen(Request(url, headers={'User-Agent': 'Mozilla'}))
...
def process_title_id(title_id, title_key, name=None, region=None, output_dir=None, retry_count=3, onlinetickets=False, patch_demo=False,
                     patch_dlc=False, simulate=False, tickets_only=False, url='http://ccs.cdn.c.shop.nintendowifi.net/ccs/download/{}', force_cetk=False, h3h3Productions=False):
    if name:
    ...
    if typecheck == '000e' or force_cetk:
    ...
    for i in range(content_count):
        ...
        if not download_file('{}/{}'.format(baseurl, c_id), outfname, retry_count, expected_size=expected_size):
            ...
            return
        if h3h3Productions:
            if not download_file('{}/{}.h3'.format(baseurl, c_id), outfnameh3, 1, ignore_404=True):
                print('No h3 file found, huh...')
                continue
        else:
